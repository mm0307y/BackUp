...
// 1. RestController를 사용하면 text로 출력이 나간다. (text/plain, application/json)
// 2. Controller를 사용하면 요청이 페이지로 출력이 나간다.
// 단 만일 text나 json포맷을 원하는 경우 리턴타입 앞에 @ResponseBody
@Log4j2
@Controller
public class IndexController0304 {
  ...
  // 회원가입 처리하기
  @PostMapping("/join")
  public String join(User0304 user) {
    // user.setRole("USER");
    String rawPassword = user.getPassword();// 암호화가 안된 비번임.- 시큐리티에서는 에러 발생
    // 평문으로 된 비번을 받아서 암호된 비번으로 변경한다.
    String encPassword = passwordEncoder.encode(rawPassword);
    user.setPassword(encPassword);
    userDao.userInsert(user);
    return "redirect:/loginForm";// 회원가입이 성공하면 이 요청을 리다이렉트 해준다.
  }// end of 회원가입
